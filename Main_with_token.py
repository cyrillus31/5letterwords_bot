...
def send_welcome(message):
    bot.send_message(message.chat.id, "Привет! Этот бот поможет тебе подобрать слово из ПЯТИ букв русского алфавита на основании известных букв.\n\n"
    ...
    try:
        ...
        if len(x)==0 and len(y)==0:
            ...
            with open("unfitletters{}.txt".format(message.chat.id), "w") as f:
                f.write("")
            bot.send_message(message.chat.id, "Хранилась информация о следующийх буквах: {} {}\n Архив почищен!".format(x, y))
...
def enter_the_letters(message):
    ...
    if message.text == "0":
        bot.send_message(message.chat.id, "Значит таких букв нет? Хорошо, начинаем искать подходящие слова!")
        theletters = ""
    else:
        bot.send_message(message.chat.id, "Буквы приняты! Начинаем искать подходящие слова!")
    ...
    #filter the words in thewords_list using letters_list AND unfitletters_list
        ...
        if len(answer)==0:
            bot.send_message(message.chat.id, "Похоже, что таких слов нет! Попробуйте заново")
            ...
            msg = bot.send_message(message.chat.id, "Хотите внести уточнения в этот поиск? да/нет")
        ...
        print("Ууууупс!")
        print (err)
        bot.send_message(message.chat.id, "Ууууупс! Что-то пошло не так.\n\n Попробуйте заново!")
        bot.send_message(message.chat.id, "Шаг 1 из 3: \nВведите слово заменяя неизвестные буквы знаком '+' Например: в+л+а")
