...
def restart_search_or_help(message):
    ...
    bot.send_message(
        message.chat.id,
        """Этот бот помогает играть в игру 5букв/ wordle
Отправь набор из пяти символов, который будет включать в себя известные тебе буквы Кириллицы (в соответствии с известным тебе расположением) и любые другие символы (кроме запятой) вместо неизвестных.
Затем, затем ты можешь поставить запятую и указать набор букв, который точно есть в этом слове, а после еще одной запятой набор тех, которых точно нет.
...
!!! Важно: обе запятые должны стоять даже если вы хотите указать только нужные буквы или только ненужные!
